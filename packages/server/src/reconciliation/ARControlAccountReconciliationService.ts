// packages/server/src/reconciliation/ARControlAccountReconciliationService.ts
// AR CONTROL ACCOUNT RECONCILIATION (SERVER)
//
// PHASE 9 — STEP 3
//
// Role:
// - Compute AR subledger vs Ledger control account reconciliation per currency
...
export type ARReconciliationInput = {
...
/**
 * Provider for authoritative AR subledger exposure per currency,
 * already expressed in MINOR units (integer).
 *
 * IMPORTANT:
 * - Exposure must be computed from AR events/read models only.
 * - Snapshot-aware if snapshotId is provided.
 */
export type ARSubledgerExposureProvider = {
...
export class ARControlAccountReconciliationService {
  constructor(
    private readonly ledger: LedgerControlAccountProvider,
    private readonly ar: ARSubledgerExposureProvider
  ) {
    if (!ledger) throw new Error('ARControlAccountReconciliationService requires ledger provider');
    if (!ar) throw new Error('ARControlAccountReconciliationService requires AR exposure provider');
  }

  /**
   * Generate an audit-grade reconciliation report (deterministic).
   */
  async reconcile(input: ARReconciliationInput): Promise<ControlAccountReconciliationReport> {
    ...
    const [ledgerRows, arRows] = await Promise.all([
      ...
      this.ar.getARNetMinorByCurrency({
        periodFrom: input.periodFrom,
        periodTo: input.periodTo,
        asOf: input.asOf,
        snapshotId: input.snapshotId ?? null,
    ...
    const arMap = new Map<string, MoneyMinorAmount>();
    for (const r of arRows) {
      if (!r.currency || r.currency.trim().length === 0) {
        warnings.push('AR provider returned empty currency code (ignored).');
        continue;
      }
      arMap.set(r.currency, normalizeMinor(r.subledgerNetMinor));
    }

    const currencies = new Set<string>([...ledgerMap.keys(), ...arMap.keys()]);

    const lines: CurrencyReconciliationLine[] = [];
    for (const currency of currencies) {
      const ledgerControlNetMinor = ledgerMap.get(currency) ?? 0;
      const subledgerNetMinor = arMap.get(currency) ?? 0;
      ...
      if (!arMap.has(currency)) {
        notes.push('AR subledger exposure missing for this currency; treated as 0.');
      }
