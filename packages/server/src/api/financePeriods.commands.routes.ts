// packages/server/src/api/financePeriods.commands.routes.ts
...
export function createFinancePeriodsCommandRoutes(): Router {
  const router = Router();
  ...
  router.post(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        return res.status(200).json({ periodId: event.periodId });
      } catch (err: any) {
        return res.status(400).json({
